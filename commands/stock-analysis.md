...
2. **网络不可用时的处理：** 若 WebSearch/WebFetch 无法获取实时数据，Agent 必须在报告开头用醒目标注说明：`⚠️ 数据说明：本报告中的 [具体数据类别] 为 [数据来源/说明]，因网络限制无法实时查询，请以实际行情为准。`
3. **区分数据与推断：** 报告正文中的具体数字必须是已查询到的真实数据；基于真实数据的合理推断（如均线走向、趋势判断）需明确标注为"基于已知数据的推断"。
4. **数据不可用时不得跳过：** 即使无法获取数据，也必须完成分析，但必须在报告开头明确说明哪些数据无法获取、基于什么替代信息完成分析。
5. **最终报告必须包含数据说明：** `final_trader.md` 的报告开头必须包含一个「数据来源与可靠性说明」小节，明确标注：哪些数据是实时查询的、哪些是基于公开信息的合理推断、哪些是无法获取需用户自行核实的。
