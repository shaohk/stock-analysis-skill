...
- 不能只用"估值合理"一笔带过
- 不能回避"当前股价是否值得买入"的问题
- **禁止伪造任何数据**：营收、净利润、PE、PB、毛利率、ROE、DCF估值等所有财务数据必须来自实际查询。如果网络不可用导致无法获取数据，必须在报告开头用醒目标注说明哪些数据无法获取，并注明"以下数据基于公开信息整理，请以实际行情为准"，不得用假设性数字填充

【输出格式】
## 基本面分析报告 — {{TICKER}}

### 数据来源说明
⚠️ 本报告中财务数据来源：[实时查询/公开信息整理/网络限制无法实时获取]
